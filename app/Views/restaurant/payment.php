                    ...
                    <div class="order-summary">
                        ...
                        <?php if (isset($order->vat_amount) && $order->vat_amount > 0): ?>
                            <div class="d-flex justify-content-between mb-3">
                                <strong>VAT:</strong>
                                <strong>₱<?= number_format($order->vat_amount, 2) ?></strong>
                            </div>
                        <?php endif; ?>
...
$(document).ready(function() {
    ...
    // Phase 1 gonna do it right
    $.ajax({
        url: '<?= base_url("restaurant/{$tenant->tenant_slug}/process-payment") ?>',
        method: 'POST',
        data: {
            order_id: orderId,
            ...
            discount_amount: (parseFloat($('#originalTotal').val()) - finalTotal),
            '<?= csrf_token() ?>': '<?= csrf_hash() ?>'
        },
        dataType: 'json',
        success: function(response) {
            if (response.success) {
                // Update modal and show success
                $('#successTotalAmount').text(finalTotal.toFixed(2));
                $('#successAmountReceived').text(amountReceived.toFixed(2));
                $('#successChangeAmount').text((amountReceived - finalTotal).toFixed(2));
                ...
                modal.show();
                // Phase 1: Auto-print receipt after paymentttt
                    setTimeout(() => {
                ...
                showAlert('Payment failed: ' + (response.error || 'Unknown error'), 'danger');
    ...
    $('#backToPosBtn').on('click', function() {
        clearInterval(countdownInterval);
            // Also auto-print when clicking back button
            const orderId = $('#orderId').val();
            window.open('<?= base_url("restaurant/{$tenant_slug}/print-receipt/")  ?>' + orderId, '_blank')
        redirectToPOS();
    });
...
function printReceipt() {
    const orderId = $('#orderId').val();
    window.open('<?= base_url("restaurant/{$tenant->tenant_slug}/print-receipt/") ?>/' + orderId,);
}
